...
const ListSong = () => {
  ...
  const fetchSongs = async () => {
    try {
      ...
      toast.error('Error fetching songs list')
    } finally {
      setLoading(false)
    }
  }
  ...
  const removeSong = async (id, title) => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        toast.success(response.data.message || `Song "${title}" removed`)
      } else {
        toast.error((response.data && response.data.message) || 'Failed to remove song')
        setData(prevData)
      }
    } catch (error) {
  ...
  if (loading && data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center pt-24 gap-4">
        ...
        <p className="text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider text-xs">Loading tracks catalog...</p>
      </div>
    )
  }
  ...
  return (
    <div className="pr-5 sm:pr-12 animate-fadeIn">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-black text-gray-900 dark:text-white uppercase">Tracks Catalog</h2>
          <p className="text-xs text-gray-500 dark:text-[#00FF5B] font-bold uppercase tracking-wide mt-1">Review all uploaded music tracks, edit metadata, or purge items.</p>
        </div>
        <button 
          onClick={fetchSongs} 
      ...
      {data.length === 0 ? (
        <div className="bg-white dark:bg-[#0c1510] border-2 border-black dark:border-[#00FF5B] border-dashed p-12 text-center text-gray-500 dark:text-gray-400 font-bold">
          No songs found. Click "Add Song" to upload your first track!
        </div>
      ) : (
            ...
            <tbody className="text-sm font-semibold text-gray-700 dark:text-gray-300">
              {data.map((item) => (
                <tr key={item._id} className="border-b-2 border-gray-100 dark:border-zinc-800 last:border-b-0 hover:bg-gray-50/50 dark:hover:bg-zinc-800/20 transition-colors">
                  ...
                  <td className="p-4 font-medium text-gray-600 dark:text-gray-400">{item.album || 'Single'}</td>
                  <td className="p-4 font-mono text-xs">{item.duration}</td>
                  <td className="p-4 flex items-center justify-center gap-3">
                    <Link 
                      to={`/edit-song/${item._id}`} 
                    ...
                    <button 
                      onClick={() => removeSong(item._id, item.title)} 
